...
with st.sidebar:
    ...
    st.markdown("> **说明：** 规格决定了最后拼豆板的物理大小。颗粒数越多，细节越丰富，但也越耗费精力和材料。")
    ...
    st.divider()
    st.subheader("3. 色彩处理")
    st.markdown("> **说明：** 现实色彩千千万，但实物拼豆颜色有限。这里控制色彩如何归纳。")
    color_limit = st.slider("色彩精简度 (色数)", 2, 60, 25, help="将原图合并压缩成多少种主色调。数字越小画面越简洁、越容易采购豆子。")
    saturation = st.slider("色彩饱和度增强", 0.5, 2.0, 1.2, help="拼豆实物颜色通常比电脑屏幕鲜艳。建议稍微拉高（>1.0）使成品更生动好看。")
    st.divider()
    st.subheader("4. 视图辅助")
    st.markdown("> **说明：** 影响屏幕预览的排版模式，方便拼豆时对齐。")
...
if uploaded_file is not None:
    ...
    if show_grid or show_labels:
        ...
        if show_grid:
            ...
            for y in range(0, res.height + 1, scale):
                ...
                draw.line([(0, y), (res.height, y)], fill=(120,120,120), width=width)
    ...
    with col1:
        ...
        with st.expander("🛠️ 展开/收起 像素级色彩微调工具"):
            ...
            with col_slice1:
                active_y = st.slider("🔍 定位行数 (Y 轴)", 0, target_h - 1, 0)
            with col_slice2:
                active_x = st.slider(f"🔍 定位列数 (X 轴)", 0, target_w - 1, 0)
            with col_slice3:
                # 允许用户自由设定前后查看的格点范围（即你想看左右几颗）
                view_range = st.slider("👀 局部寻位视野半径", 1, 15, 3, help="控制目标周围看多少颗豆子。默认看周围各3颗豆子，共25颗以上的矩阵。")

            # 🔴 计算基于中心点微缩的寻点网格
            start_x = max(0, active_x - view_range)
            end_x = min(target_w - 1, active_x + view_range)
            start_y = max(0, active_y - view_range)
            end_y = min(target_h - 1, active_y + view_range)
            
            w_box = end_x - start_x + 1
            h_box = end_y - start_y + 1
            
            st.markdown(f"**👁️ 锁定坐标附近范围的真实豆子排布 (中心红色高亮为 $X={active_x}, Y={active_y}$)：**")
            ...
            for local_y, y in enumerate(range(start_y, end_y + 1)):
                for local_x, x in enumerate(range(start_x, end_x + 1)):
                    ...
                    box_draw.rectangle([local_x * 30, local_y * 30, (local_x + 1) * 30 - 1, (local_y + 1) * 30 - 1], fill=color, outline=border, width=width)
            
            # 显示微局部
            st.image(box_img)
            st.caption("👆 调整视野半径可无缝扩大或收紧搜寻范围，红框永远是您锁定的中央豆子。")
            
            st.markdown("---")
            
            # 视觉色块选色
            st.markdown("**🎨 选择下方色库中的图标替换当前锁定格子的色彩：**")
            color_scope = st.radio("选用色彩库作用域", ["当前画布已用色", "MARD 221 全库"], horizontal=True)

            if color_scope == "当前画布已用色":
                target_mid_list = [item['色号'] for item in beads_data]
            else:
                target_mid_list = list(MARD_221.keys())

            cols_per_row = 8  # 剔除文字后，一行可以塞下更多纯色块
        ...
        # --- 导出功能 ---
        if st.button("🚀 导出完整资料包（到本地文件夹）"):
            folder_name = datetime.now().strftime("%y%m%d%H%M") 
            if not os.path.exists(folder_name):
                os.makedirs(folder_name)
            
            img_orig.save(f"{folder_name}/original.png")
            
            list_height = 50 + (len(beads_data) // 4 + 1) * 40
            ...
            final_export.save(f"{folder_name}/pixel_pattern_with_list.png")
            st.success(f"✅ 导出成功！保存路径: {os.path.abspath(folder_name)}")
...
st.divider()
st.caption("MARD 221 Expert System v2.5 | 像素局部视野微调与折叠编辑库版")